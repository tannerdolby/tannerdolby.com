...
{{ preview }} In 11ty, when you create [global data files](https://www.11ty.dev/docs/data-global/) inside the `_data/` directory. The data files in `_data` can be accessed by any template in the project structure. These global data files can be `.json` or values from `module.exports` as a `.js` file.
...
<h2 id="using-layouts" class="dir-link h2-5">Connect the markdown file to a layout {% directlink "using-layouts" %}</h2>
