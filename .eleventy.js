...
module.exports = (eleventyConfig) => {
  ...
  eleventyConfig.addShortcode("jobInfo", function (title, duration) {
    console.lo
    return (
      `<div class="experience-container">
        <p class="job-title">${title}</p>
        <p class="job-duration">${duration}</p>
      </div>`
    );
  });
  ...
  eleventyConfig.addShortcode("img", async function ({ src, alt, width, height, widths, cls, imgDir, sizes = "100vw" }) {
